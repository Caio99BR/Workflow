...
def send_pr_comment(state):
    ...
    for comment in comments:
        if comment.get("body", "").startswith("[CI]"):
    ...
    try:
        if bot_comment_id:
            ...
            if r.status_code not in (200, 201):
                ...
                print(f"[INFO] PR comment updated successfully: {state}")
        else:
            # Create new comment
            r = requests.post(api_url, headers=headers, json=data, timeout=10)
            if r.status_code not in (200, 201):
                ...
                print(f"[INFO] PR comment posted successfully: {state}")
    except Exception as e:
